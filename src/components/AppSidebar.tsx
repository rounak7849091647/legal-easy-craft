...
import { FileText, Users, Calculator, Home, Settings, LogIn, LogOut, LayoutDashboard, User, MessageSquare, Globe } from 'lucide-react';
  ...
  return <Sidebar collapsible="icon" className="border-r border-sidebar-border">
      ...
      <SidebarContent className="px-2">
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {mainItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={currentPath === item.href} className="hover:bg-sidebar-accent">
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {dashboardItems.map(item => <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild isActive={currentPath === item.href || currentPath.startsWith('/dashboard')} className="hover:bg-sidebar-accent">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {serviceItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={currentPath === item.href} className="hover:bg-sidebar-accent">
                    <Link to={item.href} className="flex items-center gap-3">
                      <item.icon size={18} className="text-muted-foreground" />
                      {!isCollapsed && <span className="text-sm">{item.title}</span>}
      ...
      <SidebarFooter className="p-3">
        <SidebarMenu>
          ...
          {user && !isCollapsed && <div className="flex items-center gap-3 px-2 py-2 mb-2 rounded-lg bg-sidebar-accent/50">
        ...
        <SidebarMenuItem>
              <SidebarMenuButton asChild className="hover:bg-sidebar-accent">
                <Link to="/auth" className="flex items-center gap-3">
                  ...
                  {!isCollapsed && <span className="text-sm">Login / Sign Up</span>}
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>}
